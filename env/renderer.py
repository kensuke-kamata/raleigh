...
class Renderer:
    def __init__(self, map, goal, walls):
        self.map = map
        self.goal = goal
        self.walls = walls
        self.ys = len(self.map)
        self.xs = len(self.map[0])
    ...
    def render_v(self, v=None, policy=None, print=True):
        ...
        for y in range(ys):
            for x in range(xs):
                ...
                if (v is not None) and state not in self.walls:
                    if print:
                ...
                if policy is not None and state not in self.walls:
